...
def checkFuel(current_fuel : float) -> str:
    if current_fuel >= 0.99:
        return "F"
    elif current_fuel <= 0.01:
        return "E"
    else:
        return f"{int(round((current_fuel * 100)))}%"                   # round() 會回傳該浮點數最近之整數



def main() -> None:
    current_fuel : float = getFuel()

    print(checkFuel(current_fuel))
